...
my @flashers;
my $simultaneous_flash_step = int 0;
for ( my $step = 1; $simultaneous_flash_step == 0; $step++ ) {
   print "\nStep $step";
   ...
   my $all_values; # put all values in a string; if all F, simultaneous flash
   foreach my $row ( sort num keys %tracker ) {
      #print "\n";
      foreach my $col ( sort num keys %{$tracker{$row}} ) {
   ...
   #print "\n$all_values\n";
   if ( $all_values =~ /^F+$/ ) {
      $simultaneous_flash_step = $step;
   }
}

print "\n\nNumber of flashes: $num_flashes\n";
print "Steps until simultaneous flash: $simultaneous_flash_step\n\n";
